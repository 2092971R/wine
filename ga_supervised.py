...
This code runs the several genetic algorithms used to determine the best attributes to use in the supervised learning techniques
...
def fitness(x):
    if sum(x) == 0:
        ...
        return sum(store)/float(len(store))
...
print("The best attibutes to be used in the Decision tree are", parameters, "giving an average accuracy score of", best[len(best)-1][0])

#for the decision tree with binary quality outcomes
   
wine = pd.read_csv("winequality-red.csv")
wine["binary quality"] = np.where(wine["quality"]<6, 0, 1)
names = list(wine.columns.values)
names.remove('quality')
names.remove('binary quality')
target_name = 'binary quality'
drop = ['quality', 'binary quality']
model = DecisionTreeClassifier()
...
print("The best attibutes to be used in the Decision tree (with a binary outcome) are", parameters, "giving an average accuracy score of", best[len(best)-1][0])
